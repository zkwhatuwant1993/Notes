...
- [Java Web高级编程学习笔记：阅读API和JSR等Java EE文档](#java-webapijsrjava-ee)
    ...
    - [九、 使用WebSocket进行通信：全双工通信（服务器推送技术）](#websocket)
        - [9.1 从AJAX到WebSocket](#91-ajaxwebsocket)
            - [9.1.1 问题：从服务器获取的数据什么时候可用](#911)
            - [9.1.2 解决方案1:轮询](#912-1)
            - [9.1.3 方案2：长轮询](#913-2)
            - [9.1.4 方案3:分块编码](#914-3)
            - [9.1.5 方案4：Applet或者Flash](#915-4appletflash)
            - [9.1.6 方案5：WebScoket](#916-5webscoket)
        - [9.2 了解WebSocket API](#92-websocket-api)
            - [9.2.1 HTML5的WebSocket Api](#921-html5websocket-api)
            - [9.2.2 Java API](#922-java-api)
        - [9.3 WebSocket的应用例子](#93-websocket)
...
## 九、 使用WebSocket进行通信：全双工通信（服务器推送技术）

WebScoket基于TCP协议，是HTML5的一部份。

### 9.1 从AJAX到WebSocket

#### 9.1.1 问题：从服务器获取的数据什么时候可用

HTTP 协议有一个缺陷：通信只能由客户端发起。举例来说，我们想了解今天的天气，只能是客户端向服务器发出请求，服务器返回查询结果。HTTP 协议做不到服务器主动向客户端推送信息。

浏览器可以只从服务器抓取数据，但浏览器并不知道新数据什么时候可用，而服务器知道。正如浏览器知道什么时候有新数据可以发送到服务器一样，只有服务器知道什么时候有新数据可以发送到浏览器。

例如：在web聊天应用中聊天时，只有服务器知道用户A已经发送了一条消息给B，用户B只有在浏览器客户端访问服务器时才会知道该消息的存在(如一直轮询)。

#### 9.1.2 解决方案1:轮询

轮询：以一个固定的频率，客户端向服务器请求(如AJAX)查询新数据。如果浏览器有新的数据要发送到服务器，数据将被添加到轮询请求中一同发送到服务器。如果有新数据，服务器将回复一个含有新数据的响应；如果没有，返回空(空JSON/XML或Content-Length为0)。

#### 9.1.3 方案2：长轮询

长轮询与轮询的概念基本相同，区别是服务器只有在有新数据要发送时才会响应浏览器。

长轮询带来的问题：

- 如果浏览器在服务器响应之前有新数据要发送到服务器，浏览器必须创建一个新的并行请求，或者它必须终止当前请求(此时服务器必须能够正确恢复)然后创建新的请求。
- TCP和HTTP协议都规定了连接超时。因此服务器和客户端必须周期性地关闭和重建连接。
- HTTP/1.1 中规范中有更严格的连接限制。浏览器最多只允许同时创建2个连接到相同服务器的连接。(意味着如果长用了一个长连接用记数据推送，那么只剩一个连接的可用资源)

#### 9.1.4 方案3:分块编码

#### 9.1.5 方案4：Applet或者Flash

随着移动端的兴起，该方案已经弃用(1.不安全 2. 内存敏感 3.移动端浏览器无法运行Applet或者Flash。

#### 9.1.6 方案5：WebScoket

利用http/1.1 的升级特性(Connection:Upgrade)：客户端使用HTTP和服务器进行握手，当扬完成时就可以使用任意协议。

主要的特点：

- 因为连接建立在80(ws)或者443(wss)上创建，与HTTP使用相同的端口，几乎所有的防火墙都不会阻塞该连接。
- 因为websocket使用HTTP进行握手，所以该协议可以自然地集成到网络浏览器和HTTP服务器中国。
- 因为握手完成后就以使用任意协议，所以不存在HTTP协议中浏览器对单个服务器的并法连接数的限制。
- Websockt可以安全地进行跨域连接，避免了AJAX和XMLHttpRequest的限制。
- 协议标识符是ws（如果加密，则为wss）

### 9.2 了解WebSocket API

#### 9.2.1 HTML5的WebSocket Api

#### 9.2.2 Java API

1. 客户端API
2. 服务端API：服务端API依赖于完整的客户端API，只是为服务端添加了少许类和接口。

### 9.3 WebSocket的应用例子

1. 多人游
2. 在集群中使用WebSocket进行通信。
